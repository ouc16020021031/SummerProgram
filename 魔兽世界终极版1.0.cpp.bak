...
void arrow_hurt_again(int number, Knight* who_arrow){
	vector<City>::iterator it = city.begin() + number;
	if((it->knight_in_city[red]->life -= arrow_attack) <= 0) {
				print_time();
				cout << color_name[who_arrow->color] << ' ' << who_arrow->name << ' ' << who_arrow->number << " shot and killed " << color_name[it->knight_in_city[red]->color] << ' ' << it->knight_in_city[red]->name << ' ' << it->knight_in_city[red]->number << endl;
				if( it->knight_in_city[blue]) {
					if(it->knight_in_city[blue]->name_value == wolf) {
						if(it->knight_in_city[red]->weapon[arrow] && it->knight_in_city[blue]->weapon[arrow] == NULL) {
							it->knight_in_city[blue]->weapon_[arrow] = it->knight_in_city[red]->weapon[arrow];
							it->knight_in_city[red]->weapon[arrow] = NULL;
						}
						if(it->knight_in_city[red]->weapon[bomb] && it->knight_in_city[blue]->weapon[bomb] == NULL) {
							it->knight_in_city[blue]->weapon_[bomb] = it->knight_in_city[red]->weapon[bomb];
							it->knight_in_city[red]->weapon[bomb] = NULL;
						}
						if(it->knight_in_city[red]->weapon[sword] && it->knight_in_city[blue]->weapon[sword] == NULL) {
							it->knight_in_city[blue]->weapon_[sword] = it->knight_in_city[red]->weapon[sword];
							it->knight_in_city[red]->weapon[sword] = NULL;
						}
					}
					it->knight_in_city[blue]->win_flag = true;
					it->knight_in_city[blue]->fight_flag = true;
				}
				delete it->knight_in_city[red];
				it->knight_in_city[red] = NULL;
	}else {
		print_time();
		cout << color_name[who_arrow->color] << ' ' << who_arrow->name << ' ' << who_arrow->number << " shot"<< endl;
		if(!--(it->knight_in_city[red]->weapon[arrow]->attack_times)) {
				delete it->knight_in_city[red]->weapon[arrow];
				it->knight_in_city[red]->weapon[arrow] = NULL;
			}
	}
}

int arrow_hurt(int number, Knight* who_arrow) {
	int i = white;
	vector<City>::iterator it = city.begin() + number;
	while(i++ < 2) {
		if(it->knight_in_city[i] && it->knight_in_city[i]->color != who_arrow->color) {
			if((it->knight_in_city[i]->life -= arrow_attack) <= 0) {
				...
				if(i==blue && it->knight_in_city[blue]->weapon[arrow]) {
					arrow_hurt_again(number - 1, it->knight_in_city[blue]);
					if(!--(it->knight_in_city[blue]->weapon[arrow]->attack_times)) {
				delete it->knight_in_city[blue]->weapon[arrow];
				it->knight_in_city[blue]->weapon[arrow] = NULL;
			}
					...
					delete it->knight_in_city[blue];
				    it->knight_in_city[blue] = NULL;
					return 0;
				}
...
void City::min_40() {
	...
	if(knight_in_city[ftemp] == NULL && knight_in_city[temp]) {
		...
		if(knight_in_city[temp]->fight_flag&&yell_flag)knight_in_city[temp]->yell(number);
	}
